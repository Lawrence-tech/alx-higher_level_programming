...
class Rectangle(Base):
    ...
    def __init__(self, width, height, x=0, y=0, id=None):
        ...
        Raises:
               ...
               ValueError: If either of the width or height is <= 0.
               TypeError: If either the x or y is not an int.
               ValueError: If either of x or y < =.
        """
        self.width = width
        self.height = height
        self.x = x
    ...
    def update(self, *args, **kwargs):
        """Method that assigns an argument to each attribute by Non-keyword
            and key/value """
        arlist = ["id", "width", "height", "x", "y"]
        if (args and len(args) != 0):
            for arl in range(len(args)):
                if (arl == 0):
                    super().__init(args[arl])
                elif (arl < len(arlist)):
